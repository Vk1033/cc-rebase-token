...
forge script ./script/ConfigurePool.s.sol:ConfigurePoolScript --rpc-url ${ETHEREUM_SEPOLIA_RPC_URL} --account myaccount --broadcast --sig "run(address,uint64,address,address,bool,uint128,uint128,bool,uint128,uint128)" ${SEPOLIA_POOL_ADDRESS} ${ZKSYNC_SEPOLIA_CHAIN_SELECTOR} ${ZKSYNC_POOL_ADDRESS} ${ZKSYNC_REBASE_TOKEN_ADDRESS} false 0 0 false 0 0
...
forge script ./script/BridgeTokens.s.sol:BridgeTokensScript --rpc-url ${ETHEREUM_SEPOLIA_RPC_URL} --account myaccount --broadcast --sig "run(address,uint64,address,uint256,address,address)" $(cast wallet address --account myaccount) ${ZKSYNC_SEPOLIA_CHAIN_SELECTOR} ${SEPOLIA_REBASE_TOKEN_ADDRESS} ${AMOUNT} ${SEPOLIA_LINK_ADDRESS} ${SEPOLIA_ROUTER}
